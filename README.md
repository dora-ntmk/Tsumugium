...
現在のバージョンは **v4.1.0.69** です。Botが送信するメッセージとコマンド説明は日本語固定です。
...
旧`config.db`へ新しい設定カラムを追加する場合は、変更前に`BACKUP_DIR`へ末尾が`v3-latest`の移行前バックアップを自動保存します。バックアップに失敗した場合、DBを変更せず起動を中止します。
