...
class UpdateSoundBoards:
  def __init__(self, db_path, dict_manager=None):
    ...
    """)
    self._conn.execute("CREATE INDEX IF NOT EXISTS idx_soundboards_guild ON soundboards (guild_id)")
    self._conn.commit()
  ...
  def refresh(self, gid: str, token: str):
    ...
    cur = self._conn.cursor()
    cur.execute("SELECT sound_id FROM soundboards WHERE guild_id = ?", (gid,))
    rows = cur.fetchall()
    ...
    cur.executemany("INSERT INTO soundboards (guild_id, sound_id, name) VALUES (?, ?, ?)", db_insert)
    cur.executemany("DELETE FROM soundboards WHERE guild_id = ? AND sound_id = ?", db_delete)
    self._conn.commit()
    if self._dict_manager:
