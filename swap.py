...
def preprocess_text(text: str, guild_id: int, conn, emoji_ja: dict, guild, attachments, mentions=None, author_id=None) -> tuple[str, list[tuple[int, int]], str | None]:
  ...
  if author_str is not None:
    cur = conn.execute(
      "SELECT sound_id FROM dict WHERE guild_id = ? AND word = ? AND sound_id IS NOT NULL AND full_match = 1 AND (trigger_user_id IS NULL OR trigger_user_id = ?) LIMIT 1", (gid, text, author_str)
    )
  else:
    cur = conn.execute("SELECT sound_id FROM dict WHERE guild_id = ? AND word = ? AND sound_id IS NOT NULL AND full_match = 1 LIMIT 1", (gid, text))
  row = cur.fetchone()
  if row:
    return text, [], row[0]
  ...
  if author_str is not None:
    cur = conn.execute("SELECT word, sound_id FROM dict WHERE guild_id = ? AND sound_id IS NOT NULL AND full_match = 0 AND (trigger_user_id IS NULL OR trigger_user_id = ?)", (gid, author_str))
  else:
    cur = conn.execute("SELECT word, sound_id FROM dict WHERE guild_id = ? AND sound_id IS NOT NULL AND full_match = 0", (gid,))
  for word, sound_id in cur.fetchall():
    if word in text:
      return text, [], sound_id

  # 2. 優先辞書（URL処理より前に適用）
  cur = conn.execute("SELECT word, reading FROM dict WHERE guild_id = ? AND is_priority = 1 AND reading IS NOT NULL ORDER BY added_at DESC", (gid,))
  d = dict(cur.fetchall())
  if d:
  ...
  cur = conn.execute("SELECT word, reading FROM dict WHERE guild_id = ? AND is_priority = 0 AND reading IS NOT NULL ORDER BY added_at DESC", (gid,))
  d = dict(cur.fetchall())
  if d:
    segments = _apply_dict(segments, d)

  cur = conn.execute("SELECT word, reading FROM dict WHERE guild_id = '__common__' AND reading IS NOT NULL ORDER BY added_at DESC")
  d = dict(cur.fetchall())
  if d:
    segments = _apply_dict(segments, d)
